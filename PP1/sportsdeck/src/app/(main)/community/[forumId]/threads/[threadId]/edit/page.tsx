...
export default function editThreadPage(){
    ...
    const [hadPoll, setHadPoll] = useState(false);
    ...
    useEffect(() => {
    fetch(`/api/forums/${forumId}/thread/${threadId}`)
        .then(res => res.json())
        .then(data => {
            const t = data.thread;
            ...
            // Only Author can edit thread
            const payload = JSON.parse(atob(token.split(".")[1]));
            if (payload.id !== t.authorId) {
                alert("You are not authorized to edit this thread.");
                router.push(`/community/${forumId}`);
                return;
            }

            // Reload original contents
            setTitle(t.title ?? "");
            setContent(t.post?.content ?? "");
            setTags(t.tags?.map((tag: any) => tag.name) ?? []);
            ...
            if (t.poll) {
                ...
                setPollDeadline(t.poll.deadline?.slice(0, 16) ?? "");
                setPollOptions(t.poll.options?.map((o: any) => o.text) ?? ["", ""]);
            }
        });
    }, [forumId, threadId]);
    ...
    async function handleSubmit() {
        ...
        try {
            // Update thread
            const threadRes = await fetchWithAuth(`/api/forums/${forumId}/thread/${threadId}`, {
                method: "PATCH",
            ...
            if (hadPoll && !hasPoll) {
            ...
            } else if (!hadPoll && hasPoll) { // User Creates poll
                ...
                if (!pollRes.ok) {
                    ...
                    setError(pollData.error || "Thread updated but poll failed.");
                    router.push(`/community/${forumId}/threads/${threadId}`);
                    return;
                }
            } else if (hadPoll && hasPoll && pollId) {
                // User edits poll when it existed already
                // Can modify question only
                await fetchWithAuth(`/api/polls/${pollId}`, {
                    method: "PATCH",
                    headers: { "Content-Type": "application/json" },
        ...
        <div className="min-h-screen px-4 py-8">
            ...
            <div className="w-full lg:w-[60%] mx-auto">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <span className="text-[11px] text-[#6b6b8a] tracking-widest">Edit Thread</span>
                    <div className="w-12" />
                </div>
            </div>
...

            {
                hasPoll && (
                    <div className="flex flex-col gap-4 p-4 rounded-xl border border-[#00e5a0] bg-[rgba(0,229,160,0.04)] mt-2">
                        ...
                        <div>
                            <label className="text-[11px] text-[#6b6b8a] tracking-[0.5px] block mb-1.5">OPTIONS (CANNOT BE EDITED ONCE POSTED)</label>
                                ...
                                {pollOptions.map((option, i) => (
                                    <div key={i} className="flex items-center gap-2">
                                        <input
                                            ...
                                            className={`flex-1 px-3.5 py-2.5 bg-white dark:bg-[#111118] rounded-lg text-gray-900 dark:text-[#f0f0f8] text-sm outline-none transition-colors placeholder:text-[#6b6b8a] ${
                                                hadPoll ? "border border-[#ff4c6a] opacity-50 cursor-not-allowed" : "border border-gray-200 dark:border-[#1e1e2e] focus:border-[#00e5a0]"
                                            }`}
                                        />

                                        {/* Remove button */}
                                        {pollOptions.length > 2 && !hadPoll && (
                                            <button
                            ...
                            <div>
                                <label className="text-[11px] text-[#6b6b8a] tracking-[0.5px] block mb-1.5">DEADLINE (CANNOT BE EDITED ONCE POSTED)</label>
                                <input
                                    type="datetime-local"
                                    value={pollDeadline}
                                    disabled={hadPoll}
